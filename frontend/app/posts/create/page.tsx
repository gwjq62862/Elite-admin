...
const handleInputChange = (
    ...
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!ALLOWED_IMAGE_TYPES.includes(file.type)) {
        setToast({
          isVisible: true,
          content:
            "Custom Thumbnail အတွက် JPG, PNG သို့မဟုတ် GIF ကိုသာ ခွင့်ပြုပါသည်။",
          color: "bg-red-500/90",
        });
        return;
      }
  ...
  const handleCreatePost = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.title || !formData.youtubeUrl) {
      setToast({
        isVisible: true,
        content: "Title နှင့် YouTube URL များ ဖြည့်သွင်းရန် လိုအပ်ပါသည်။",
        color: "bg-red-500/90",
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        "https://elite-admin-production.up.railway.app/api/posts",
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            "Post Creation ပြုလုပ်ရာတွင် အမှားအယွင်း ရှိခဲ့ပါသည်။"
        );
      }
      ...
      setToast({
        isVisible: true,
        content: "Post အောင်မြင်စွာ ဖန်တီးပြီးပါပြီ။",
        color: "bg-green-500/90",
      });
      ...
      setToast({
        ...
        content:
          error.message ||
          "ကွန်ယက် ချိတ်ဆက်မှု ပြဿနာ သို့မဟုတ် Server Error ရှိခဲ့ပါသည်။",
        color: "bg-red-500/90",
      });
    } finally {
      setIsLoading(false);
